...
import {
  barHistorySize, getKoiState, isHistoryRange, selectedBarsSymbol,
  visibleBars, barDirection,
} from '@state';
...
const Holdings = ({ strategy }: HoldingsProps) => {
  ...
  return (
    <div id="holdings" className="holdings-card relative flex flex-col min-w-0 break-words bg-white dark:bg-off-black w-full mb-6 shadow-lg rounded">
      {/* Card Header */}
          ...
          <div className="flex flex-col items-baseline pb-8">
            ...
            <div className="relative flex text-center text-sm leading-none font-bold text-gray-500">
              {strategy.contracts.length > 0 && (
                <>
                  {/* Market Contracts */}
                  {isCryptoDataList(strategy.contracts)
                    ? strategy.contracts.map(c => (
                      <div className={`relative flex pl-4 pr-4 flex-col justify-center items-center`}>
                        <span onClick={() => setSelectedSymbol(`${c.market}-${c.currency}`)} className={`z-10 w-auto cursor-pointer flex text-black dark:text-white`}>{`${c.market}-${c.currency}`}</span>
                        {(selectedSymbol === `${c.market}-${c.currency}`) && <span className="z-0 w-8 h-1 -mt-1 bg-green-200 dark:bg-gren-800"></span>}
                      </div>
                    )) : (
                      strategy.contracts.map(c => (
                        <div className={`relative flex pl-4 pr-4 flex-col justify-center items-center`}>
              ...
              <table className="relative bars-table items-center w-full bg-transparent border-collapse">
                ...
                <tbody>
                  {bars.data.map((rowData, i) => (
                    <tr id={`${selectedSymbol}_` + (rowData[bars.columns.findIndex(col => col === 'date')] || `${i}`).replace(/[\:\/ ]/g, '_')}>
