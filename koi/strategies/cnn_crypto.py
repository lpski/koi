...
class Strategy(StrategyInterface):
    ...
    name = 'CNN-Crypto'
    description = 'Pure CNN based crypto predictions'
    ...
    def determine_next_move(self, dfs: Dict[str, pd.DataFrame]) -> Tuple[List[Decision], Dict[str, List[Prediction]]]:
        ...
        #         better_opp_available = any([(opp.confidence >= lo_buy_conf and opp.confidence >= self.cnn_manager.conf_threshold[opp.contract.symbol] and opp.contract.symbol != contract.symbol) for opp in buy_opportunities])
    ...
    def fit(self, dfs: Dict[str, pd.DataFrame]):
        ...
        for sym, df in dfs.items():
            ...
            cnn_pred = self.cnn_manager.predict(df, sym)
    ...
    def should_buy(self, df: pd.DataFrame, symbol: str) -> Tuple[bool, float, str]:
        ...
        """
        # if symbol == 'BTC-USD': return (True, 1, 'test')
    ...
    def should_sell(self, df: pd.DataFrame, symbol: str) -> Tuple[bool, float, str]:
        ...
        if cnn_pred.direction == Direction.Up:
            ...
            return (not threshold_met, cnn_pred.confidence, f'{"over" if threshold_met else "under"} CNN high confidence threshold')
