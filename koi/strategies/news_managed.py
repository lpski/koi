...
class Strategy(StrategyInterface):
    ...
    name = 'News-Managed'
    description = '1 minute rapid news actively updated portfolio - trend following'
    targets = [StrategyTarget.volume, StrategyTarget.arima]
    trade_config = TradeConfig(300, 0.03, '7 D', 0.5)
    ...
    def determine_next_move(self, dfs: Dict[str, pd.DataFrame]) -> Tuple[List[Decision], Dict[str, List[Prediction]]]:
        ...
        for c in self.contracts:
            ...
            if p.has_stock:
                ...
                if sell: decisions.append(Decision(Move.Sell, c, p.quantity, confidence, reason))
    ...
    async def prepare(self, train_dfs: Dict[str, pd.DataFrame] = None, analyze_dfs: Dict[str, pd.DataFrame] = None, main_loop: AbstractEventLoop = None):
        ...
        # main_loop.run_until_complete(self.analyzer.start(analyze_dfs, None, main_loop, True))
